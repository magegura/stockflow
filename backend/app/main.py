...
app = FastAPI(
    title="StockFlow API",
    version="4.0.0",
    description="Publication-ready inventory and sales backend with auth, profiles, personnel management, logs and business workflows.",
    lifespan=lifespan,
)
...
class StaffUserCreate(BaseModel):
    ...
    hierarchy_level: Literal["staff", "lead", "management"] = "staff"
...
def get_or_create_profile(db: Session, user_id: int) -> EmployeeProfile:
...
def login(payload: LoginRequest, db: Session = Depends(get_db)) -> dict:
    ...
    profile = get_or_create_profile(db, row.id)
    ...
    return {"access_token": token, "user": serialize_user_profile(row, profile)}


@app.get("/api/me")
def me(db: Session = Depends(get_db), user: dict = Depends(get_current_user)) -> dict:
    current = db.get(User, user["id"])
    if not current:
        raise HTTPException(status_code=404, detail="User not found")
    profile = get_or_create_profile(db, current.id)
    db.commit()
    return serialize_user_profile(current, profile)
...
def update_profile(
    ...
    current.email = payload.email.lower().strip()
    ...
    try:
        create_audit_log(
            ...
            message=f"Profile updated for {current.name}",
            entity_type="user",
            entity_id=current.id,
            created_by=current.id,
            details={"email": current.email, "department": profile.department, "title": profile.title},
            created_at=profile.updated_at,
        )
        db.commit()
    except IntegrityError as exc:
        db.rollback()
        raise HTTPException(status_code=400, detail="Email must be unique") from exc

    return {"message": "Profile updated", "user": serialize_user_profile(current, profile)}
...
def list_users(
    ...
    return {"items": [serialize_user_profile(row[0], row[1]) for row in rows]}
...
def create_user(
    ...
    try:
        ...
        create_audit_log(
            ...
            created_by=user["id"],
            details={"email": new_user.email, "role": new_user.role, "status": payload.status},
            created_at=timestamp,
        )
        db.commit()
    except IntegrityError as exc:
    ...
    return {"message": "User created", "user": serialize_user_profile(new_user, profile)}
...
def update_user(
    ...
    profile.hierarchy_level = payload.hierarchy_level
    profile.manager_name = payload.manager_name.strip() if payload.manager_name else None
    profile.bio = payload.bio.strip() if payload.bio else None
    profile.updated_at = now_utc()
    ...
    try:
        create_audit_log(
            db,
            action="user.updated",
            message=f"User {row.name} updated",
            entity_type="user",
            entity_id=row.id,
            created_by=actor["id"],
            details={
    ...
    return {"message": "User updated", "user": serialize_user_profile(row, profile)}
...
def update_product(
    ...
    logger.info("Product updated by %s: %s", user["email"], product.sku)
...
def delete_product(
    ...
    sale_item_exists = db.scalar(select(func.count(SaleItem.id)).where(SaleItem.product_id == product_id)) or 0
    if sale_item_exists:
        raise HTTPException(status_code=400, detail="Cannot delete a product with sales history")
    ...
    create_audit_log(
        ...
        entity_id=product.id,
        created_by=user["id"],
        details={"sku": product.sku, "stock": product.stock},
        created_at=timestamp,
    )
    db.delete(product)
    db.commit()

    logger.info("Product deleted by %s: %s", user["email"], product.sku)
