...
function App() {
  ...
  const handleDeleteProduct = async (product: Product) => {
    if (!token) return
    if (!window.confirm(interpolate(t.common.deleteProductConfirm, { name: product.name }))) return
    try {
          ...
          <section className="stack-xl">
            {user.role === 'admin' ? (
              <section className="panel">
                <div className="section-head">
                  <div>
                    <h3>{editingProductId ? t.products.editProduct : t.products.createProduct}</h3>
                    <p className="muted small">{t.products.productActionsNote}</p>
                  </div>
                  {editingProductId ? <button className="ghost" onClick={resetProductForm} type="button">{t.common.cancelEdit}</button> : null}
                </div>
                <form className="form-grid" onSubmit={handleSubmitProduct}>
                  ...
                  <div className="button-row end span-full"><button type="submit">{editingProductId ? t.products.saveChanges : t.products.createProductButton}</button></div>
                </form>
              </section>
            ) : null}
                    ...
                    <article className="product-card elevated" key={product.id}>
                      ...
                      {user.role === 'admin' ? (
                        <div className="card-actions">
                          ...
                          <button className="ghost compact danger" type="button" onClick={() => void handleDeleteProduct(product)}>{t.products.delete}</button>
                        </div>
                      ) : null}
                    </article>
                  )
